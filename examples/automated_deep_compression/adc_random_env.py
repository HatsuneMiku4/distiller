...
def random_agent(env):
    ...
    env.action_space = RandomADCActionSpace(
        action_range[0],
        action_range[1],
        std=0.35,
        mean=env.amc_cfg.target_density,
    ...
    for ep in range(1000):
        ...
        for t in range(100):
            # env.render(0, 0)
            msglogger.info(
                "[episode={}:{}] observation = {}".format(ep, t, observation)
            )
            ...
            if done:
                ...
                msglogger.info(
                    "New solution found: episode={} reward={} config={}".format(
                        ep, reward, action_config
                    )
                )
                break

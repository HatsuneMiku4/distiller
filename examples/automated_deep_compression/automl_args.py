def add_automl_args(argparser, arch_choices=None, enable_pretrained=False):
    ...
    argparser.add_argument(
        '--amc', dest='AMC', action='store_true', help='AutoML Compression'
    )
    group = argparser.add_argument_group('AutoML Compression Arguments')
    group.add_argument(
        '--amc-protocol',
        choices=[
            "mac-constrained", "param-constrained", "accuracy-guaranteed",
    ...
    group.add_argument(
        '--amc-agent-algo',
        choices=[
            "ClippedPPO-continuous", "ClippedPPO-discrete", "DDPG",
            "Random-policy"
        ],

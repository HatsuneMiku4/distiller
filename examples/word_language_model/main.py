...
# <editor-fold desc=">>> General Arguments">
parser.add_argument(
    '--data',
    type=str,
...
# <editor-fold desc=">>> Distiller Arguments">
...
parser.add_argument(
    '--momentum', type=float, default=0., metavar='M', help='momentum'
)
parser.add_argument(
    '--weight-decay',
    '--wd',
    ...
    try:
        if dataset == 'wikitext2':
            ...
            dummy_input = torch.LongTensor(
                seq_len * batch_size
            ).zero_().view(-1, batch_size).to('cuda')
            hidden = model.init_hidden(batch_size)
            dummy_input = (dummy_input, hidden)
        else:
            msglogger.info(
                "Unsupported dataset (%s) - aborting draw operation" % dataset
            )
            return
        ...
        msglogger.info(
            "An error has occured while generating the network PNG image."
        )
...
lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
    optimizer, mode='min', patience=0, verbose=True, factor=0.5
)
# </editor-fold>

# <editor-fold desc=">>> Loggers & Summary">
...
# <editor-fold desc=">>> Compression Scheduler">
...
if args.compress:
    ...
    compression_scheduler = distiller.config.file_config(
        model, None, args.compress
    )

# </editor-fold>


# <editor-fold desc=">>> Train Code">
...
def train(epoch, optimizer, compression_scheduler=None):
    ...
    for batch, i in enumerate(range(0, train_data.size(0), args.bptt)):
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            msglogger.info(
                '| epoch {:3d} | {:5d}/{:5d} batches | lr {:02.4f} '
            ...
            stats = (
                ...
                OrderedDict(
                    [
                        ('Loss', cur_loss), ('Perplexity', math.exp(cur_loss)),
            ...
            distiller.log_training_progress(
                stats, model.named_parameters(), epoch, steps_completed,
...
try:
    for epoch in range(0, args.epochs):
        ...
        msglogger.info('-' * 89)
        msglogger.info(
            '| end of epoch {:3d} | time: {:5.2f}s | valid loss {:5.3f} | valid ppl {:8.2f}'
            .format(
        ...
        distiller.log_weights_sparsity(
            model, epoch, loggers=[tflogger, pylogger]
        )

        stats = (
            'Performance/Validation/',
            OrderedDict(
                [('Loss', val_loss), ('Perplexity', math.exp(val_loss))]
            )
        )
        tflogger.log_training_progress(stats, epoch, 0, total=1, freq=1)
        # </editor-fold>

        # <editor-fold desc=">>> Save (Best) Model">
...
def export_onnx(path, batch_size, seq_len):
    ...
    model.eval()
    dummy_input = torch.LongTensor(seq_len * batch_size
                                  ).zero_().view(-1, batch_size).to(device)
    hidden = model.init_hidden(batch_size)
